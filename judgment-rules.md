...
## 산업/섹터 비교 — 순위 산정 (여러 기업을 한 번에 비교할 때만)

`investment-desk`가 산업/섹터명을 입력받아 여러 기업을 병렬로 판단한 경우, 개별 보고서와 별도로 순위표를 만든다. 세 기준① ② ③이 모두 동일한 100점 Score+Gate 아키텍처(`references/scorecard_and_verdict.md`, 기준별)를 쓰므로 기준 간 Score를 직접 비교할 수 있다.

- **탈락**: 1단계 3개 기준이 모두 미부합인 기업은 순위표에서 제외하고 "탈락(투자 매력 없음)" 목록으로 별도 표기한다 — 위 "1단계" 절에서 이미 판단이 그 시점에 종료되는 기업들과 동일한 대상이다. Scope Gate에서 걸린 기업(B2C 아님)도 순위표와 분리해 "스코프 밖" 목록에 둔다.
- **순위**: 나머지 기업(1개 이상 기준에서 부합 또는 부분부합)은 **그 기업이 부합 또는 부분부합으로 판정받은 기준들 중 최고 Score**(미부합 기준의 점수는 제외) 내림차순으로 정렬한다 — 이 기업이 가장 잘 맞는 철학 하나를 대표값으로 쓴다는 뜻이다.
- **동점 처리**: Score가 같으면 부합/부분부합 기준 개수가 많은 쪽을 상위로 둔다.
- 2단계 스크리닝 체크리스트(Pass/Caution/Fail)는 순위표에 참고 컬럼으로 함께 표시하되, 순위 산정 자체에는 반영하지 않는다 — 위 "2단계" 절에 명시된 대로 이 항목들은 "사도 되는가"를 보는 리스크 점검이지 우선순위를 매기는 성격이 아니다.
